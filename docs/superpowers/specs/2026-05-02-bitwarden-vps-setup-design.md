...
Internet → :80/:443 → существующий nginx
                       ...
                       └── panda.hello-vanilla.ru → 127.0.0.1:8082 (новый vhost)
                                                      ↓
...
- DNS A-запись `panda.hello-vanilla.ru → <IP_VPS>`
...
├── docker-compose.yml
├── nginx/
│   └── panda.hello-vanilla.ru.conf.template   # vhost-шаблон с плейсхолдерами
├── scripts/
...
`nginx/panda.hello-vanilla.ru.conf.template` (плейсхолдеры `__DOMAIN__` и `__BIND_PORT__` подменяются `install.sh` через `envsubst` или `sed`):
...
panda    A    <IP_VPS>
```
Проверка: `dig +short panda.hello-vanilla.ru` возвращает IP VPS.
...
**Шаг 4 — Регистрация владельца.** Открыть `https://panda.hello-vanilla.ru` в браузере → Create Account → ввести email + мастер-пароль.
...
- `https://panda.hello-vanilla.ru/alive` → должно вернуть 200.
...
После: `https://panda.hello-vanilla.ru` → войти мастер-паролем.
...
| TLS-ошибка в браузере | `certbot certificates`, `curl -v https://${DOMAIN}` | `sudo certbot renew --force-renewal -d panda.hello-vanilla.ru && sudo systemctl reload nginx` |
...
- [ ] `dig +short panda.hello-vanilla.ru` возвращает IP VPS
- [ ] `curl -fsS https://panda.hello-vanilla.ru/alive` → 200
- [ ] `docker compose ps` показывает `bitwarden` со статусом `healthy`
- [ ] `nginx -t` без ошибок
- [ ] `certbot certificates` показывает действующий сертификат для `panda.hello-vanilla.ru`
...
- [ ] Установлены **мобильное приложение** и **браузерное расширение Bitwarden**, оба настроены на server URL `https://panda.hello-vanilla.ru`, вход работает, сейф синхронизируется
...
3. **`BW_DOMAIN` expects bare hostname, not full HTTPS URL.** The entrypoint prepends `https://` itself: `VAULT_SERVICE_URI=https://${BW_DOMAIN:-localhost}` (entrypoint.sh line 12). Setting `BW_DOMAIN=https://panda.hello-vanilla.ru` would result in `https://https://panda.hello-vanilla.ru`. Correct value: `BW_DOMAIN=panda.hello-vanilla.ru`. Note: the spec's own Section 4.1 `.env.example` already uses `DOMAIN=panda.hello-vanilla.ru` (bare hostname — correct). However, the plan file `docs/superpowers/plans/2026-05-02-bitwarden-vps-setup.md` Task 2 (around line 118) and Task 7 (around line 587) both assume `BW_DOMAIN=https://...` and strip the scheme with shell parameter expansion. Those plan sections contradict the verified upstream behaviour and must be updated before the implementer runs Task 7's `install.sh`.
